...
class AboutMessagePassing < Neo::Koan
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    #send can be used by creating a separate method send, which leads changing the the
    #format to __send__ in order to separate the confusion
  ...
  def test_sending_a_message_with_arguments
    ...
    assert_equal [3, 4, nil, 6], mc.send(:add_a_payload, 3, 4, nil, 6) #payload is still a symbol with an empty argument
  end
  ...
  def test_foo_method_are_caught
    ...
    assert_equal "Foo to you too", catcher.foo_bar #passing the method name, convert to symbol and only grabbing first 3 elements foo
